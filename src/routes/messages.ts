...
router.post('/', async (req, res) => {
  try {
    ...
    if (!leadId || !content) return res.status(400).json({ error: 'Lead ID and content are required' });
...
router.post('/ai-reply', async (req, res) => {
  try {
    ...
    if (!leadId || !message) return res.status(400).json({ error: 'Lead ID and message are required' });
    ...
    const stockList = availableStock.length > 0
      ? availableStock.map((s: any) =>
          `- ${s.name}${s.price ? ` at R${s.price.toLocaleString()}` : ''}${s.description ? ` (${s.description})` : ''} — AVAILABLE`
        ).join('\n')
