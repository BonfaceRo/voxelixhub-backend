...
router.post('/upload', async (req, res) => {
  try {
    ...
    if (!prospects || prospects.length === 0) return res.status(400).json({ error: 'No prospects provided' });
...
router.get('/', async (req, res) => {
  try {
    ...
    res.json({ prospects, total, page: parseInt(page as string), limit: parseInt(limit as string) });
  } catch (error) {
    res.status(500).json({ error: String(error) });
  }
});
...
router.post('/blast', async (req, res) => {
  try {
    ...
    if (!channel) return res.status(400).json({ error: 'channel is required (SMS or EMAIL)' });

    const where: any = { tenantId: req.user!.tenantId, status: 'PENDING' };
    if (prospectIds?.length > 0) where.id = { in: prospectIds };

    const prospects = await prisma.prospect.findMany({ where });
    if (prospects.length === 0) return res.status(400).json({ error: 'No pending prospects found' });
